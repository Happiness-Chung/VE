...
def lazy_load(fn):
    ...
    # return sd
    # 기존 lazy loading 코드
    mup = torch.load("/root/video-edit/MotionGPT/checkpoints/lit-llama/7B/lit-llama.pth", map_location="cpu")

    # 로드 후에 바로 텐서를 불러오도록 강제
    for key, value in mup.items():
        if isinstance(value, NotYetLoadedTensor):
            mup[key] = value.load()  # 또는 적절한 방식으로 텐서 로드
    return mup
