...
def render(index, motions, name, outdir='render_vis', device_id=0):
    ...
    if not os.path.exists(os.path.join(outdir, name+'{}.pt'.format(index))):
        ...
        torch.save(vertices, os.path.join(outdir, name+'{}.pt'.format(index)))
    else:
        vertices = torch.load(os.path.join(outdir, name+'.pt'.format(index)))
    ...
    imageio.mimsave(os.path.join(outdir, f'{name}_render{index}.gif'), out, fps=20)
