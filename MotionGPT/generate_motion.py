...
def main(
    ...
    tokenizer_path = Path("./checkpoints/lit-llama/7B/tokenizer.model")
    ...
    # output = '11,12,13,14,15,16,17,17,19'
    output_tokens = [token.strip() for token in output.split(',')]
    tokens = torch.tensor([int(token) for token in output_tokens if token.isdigit()]).cuda()
    print("output: ", output)
    ...
    np.save(os.path.join(args.out_dir, 'demo{}.npy'.format(index)), generated_pose)
    # imageio.mimsave(os.path.join(args.out_dir, 'demo{}.gif'.format(index)), np.array(img), fps=20)
    if args.render:
        print("Rendering...")
        render(index, generated_pose, 'demo', outdir=args.out_dir)
